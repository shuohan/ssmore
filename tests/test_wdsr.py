...
def test_wdsr():
    ...
    net_aa = WDSRB(scale=4, num_blocks=4, use_padding=False, num_k3=2).cuda()
    x = torch.rand(1, 1, 64, 64).cuda()
    print(x.shape, x.dtype)
    dot = make_dot(x, net_aa)
    path = Path('results_models')
    path.mkdir(exist_ok=True)
    dot.render(path.joinpath('wdsr_aa'))
    print(net_aa)

    net_sr = WDSRB(scale=1, num_blocks=4, use_padding=False, num_k3=2).cuda()
    dot = make_dot(x, net_sr)
    path = Path('results_models')
    path.mkdir(exist_ok=True)
    dot.render(path.joinpath('wdsr_sr'))
